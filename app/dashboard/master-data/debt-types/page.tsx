...
import { Plus, Edit, Trash2, Search, CreditCard, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DebtTypeMasterPage() {
 ...
 const [loading, setLoading] = useState<boolean>(false);
  ...
  const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/debt-types?${params}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editItem
      ? `/api/master/debt-types/${editItem.id}`
      : `/api/master/debt-types`;
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/debt-types/${item.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data Jenis Hutang</h1>
        <p className="text-muted-foreground">
          Manage debt type data for the application
        </p>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Jenis Hutang</CardTitle>
            </div>
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editItem ? "Edit Jenis Hutang" : "Add New Jenis Hutang"}
                    </DialogTitle>
                    <DialogDescription>
                  ...
                  <form onSubmit={handleSubmit}>
                    <div className="space-y-4">
                      <div>
                        <Label htmlFor="name">Jenis Hutang</Label>
                        <Input
                          id="name"
                          value={formData.name || ""}
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Jenis Hutang</TableHead>
