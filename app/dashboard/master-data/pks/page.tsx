...
import { Plus, Edit, Trash2, Search, Factory, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function PksMasterPage() {
  ...
  const [formData, setFormData] = useState<{ name: string; address?: string; phone?: string }>({ name: "", address: "", phone: "" });
 ...
 const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/pks?${params}`);
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/pks/${item.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data PKS</h1>
        <p className="text-muted-foreground">
          Manage PKS (Pabrik Kelapa Sawit) data for the application
        </p>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>PKS</CardTitle>
            </div>
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogTrigger asChild>
                  <Button onClick={() => { setEditItem(null); setFormData({ name: "", address: "", phone: "" }); }}>
                ...
                <DialogContent>
                  ...
                  <form onSubmit={handleSubmit}>
                    <div className="space-y-4">
                      <div>
                        <Label htmlFor="name">Nama PKS</Label>
                        <Input
                          id="name"
                          value={formData.name || ""}
