...
export default function EmployeeDepartmentMasterPage() {
  ...
  const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/employee-departments?${params}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editItem
      ? `/api/master/employee-departments/${editItem.id}`
      : `/api/master/employee-departments`;
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/employee-departments/${item.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data Departemen Karyawan</h1>
        <p className="text-muted-foreground">
          Manage employee department data for the application
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Departemen Karyawan</CardTitle>
            </div>
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editItem ? "Edit Departemen Karyawan" : "Add New Departemen Karyawan"}
                    </DialogTitle>
                    <DialogDescription>
        ...
        <CardContent>
          <div className="flex items-center gap-2 mb-4">
            ...
            <Input
              placeholder="Search employee departments..."
              value={searchTerm}
