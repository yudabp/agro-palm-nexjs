...
export default function EmployeeGroupMasterPage() {
 ...
 const [searchTerm, setSearchTerm] = useState<string>("");
  ...
  const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/employee-groups?${params}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editItem
      ? `/api/master/employee-groups/${editItem.id}`
      : `/api/master/employee-groups`;
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/employee-groups/${item.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data Golongan Karyawan</h1>
        <p className="text-muted-foreground">
          Manage employee group data for the application
        </p>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Golongan Karyawan</CardTitle>
            </div>
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editItem ? "Edit Golongan Karyawan" : "Add New Golongan Karyawan"}
                    </DialogTitle>
                    <DialogDescription>
