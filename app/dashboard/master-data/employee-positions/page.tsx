...
import { Plus, Edit, Trash2, Search, Users, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function EmployeePositionMasterPage() {
  ...
  const [formData, setFormData] = useState<{ name: string; level?: string }>({ name: "", level: "" });
  ...
  const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/employee-positions?${params}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editItem
      ? `/api/master/employee-positions/${editItem.id}`
      : `/api/master/employee-positions`;
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/employee-positions/${item.id}`, {
        method: "DELETE",
      });
 ...
 useEffect(() => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data Jabatan Karyawan</h1>
        <p className="text-muted-foreground">
          Manage employee position data for the application
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Jabatan Karyawan</CardTitle>
            </div>
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogTrigger asChild>
                  <Button onClick={() => { setEditItem(null); setFormData({ name: "", level: "" }); }}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editItem ? "Edit Jabatan Karyawan" : "Add New Jabatan Karyawan"}
                    </DialogTitle>
                    <DialogDescription>
        ...
        <CardContent>
          <div className="flex items-center gap-2 mb-4">
            ...
            <Input
              placeholder="Search employee positions..."
              value={searchTerm}
