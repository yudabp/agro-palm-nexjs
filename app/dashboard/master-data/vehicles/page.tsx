...
import { Plus, Edit, Trash2, Search, Truck, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function VehicleMasterPage() {
 ...
 const [editItem, setEditItem] = useState<MasterDataItem | null>(null);
  ...
  const [formData, setFormData] = useState<{ noPol: string }>({ noPol: "" });
  ...
  const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/vehicles?${params}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editItem
      ? `/api/master/vehicles/${editItem.id}`
      : `/api/master/vehicles`;
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/vehicles/${item.id}`, {
        method: "DELETE",
      });
 ...
 useEffect(() => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data Kendaraan</h1>
        <p className="text-muted-foreground">
          Manage vehicle data for the application
        </p>
      </div>
            ...
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogTrigger asChild>
                  <Button onClick={() => { setEditItem(null); setFormData({ noPol: "" }); }}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editItem ? "Edit Kendaraan" : "Add New Kendaraan"}
                    </DialogTitle>
                    <DialogDescription>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setFormData(prev => ({ ...prev, noPol: e.target.value }))}
                          required
                        />
                      </div>
                    </div>
          ...
          {loading ? (
            ...
            <Table>
              ...
              <TableBody>
                {data.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={canWrite ? 3 : 2} className="text-center">
                      No data found
                    </TableCell>
                  </TableRow>
                ) : (
