...
import { Plus, Edit, Trash2, Search, DollarSign, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function BkkExpenseCategoryMasterPage() {
 ...
 const [data, setData] = useState<MasterDataItem[]>([]);
  ...
  const fetchData = async (page?: number, limit?: number) => {
    ...
    try {
      ...
      const response = await fetch(`/api/master/bkk-expense-categories?${params}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editItem
      ? `/api/master/bkk-expense-categories/${editItem.id}`
      : `/api/master/bkk-expense-categories`;
  ...
  const handleDelete = async (item: MasterDataItem) => {
    ...
    try {
      const response = await fetch(`/api/master/bkk-expense-categories/${item.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Master Data Kategori Pengeluaran BKK</h1>
        <p className="text-muted-foreground">
          Manage BKK expense category data for the application
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Kategori Pengeluaran BKK</CardTitle>
            </div>
            {canWrite && (
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>
                      {editItem ? "Edit Kategori Pengeluaran BKK" : "Add New Kategori Pengeluaran BKK"}
                    </DialogTitle>
                    <DialogDescription>
        ...
        <CardContent>
          <div className="flex items-center gap-2 mb-4">
            ...
            <Input
              placeholder="Search BKK expense categories..."
              value={searchTerm}
